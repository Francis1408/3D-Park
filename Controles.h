...
double angulocamera=0;//s� para o panoramica
...
void teclado(unsigned char key, int x, int y) {
    if (cam.mode == sphericCam) {//pois no modo esferico a direcao do look at � contraria ao movimento
		cam.lx = -cam.lx;
		cam.lz = -cam.lx;
	}
	else {
    ...
    switch (key) {
        //"Esta faltandu identaciooon nu case" - �rick Jacquin Brazilian-French chef
        case 27:
            exit(0);
            break;
        case '1':
